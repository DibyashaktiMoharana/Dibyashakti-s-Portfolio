...
const SkillsWorksSection = ({ projects }: SkillsWorksSectionProps) => {
  ...
  return (
    <div className="h-auto grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-2">
      <BentoCard
        classes="col-span-2 pt-6 px-6 pb-4 font-med"
        data-redirect="true"
        data-cursor-scale="true"
        onClick={() => router.push("/recent-work")}
      >
        <div className="flex items-center justify-between mb-4">
          <p className="text-2xl font-deb text-white">Recent works</p>
          <RedirectButton size={20} />
        </div>
        <SemiCircleCarousel projects={projects} autoRotateInterval={3000} />
      </BentoCard>
      <BentoCard classes="col-span-2 p-8 font-med mb-4 md:mb-0">
        <div className="flex items-center justify-between mb-6">
          <p className="text-2xl font-deb text-white">Skills</p>
        </div>
        <AllSkills />
      </BentoCard>
    </div>
